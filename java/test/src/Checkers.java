/**
 * Created by Sudhabindu on 20-Jun-17.
 */
...
public class Checkers extends JPanel {
    ...
    public Checkers() {
        ...
        saveButton.setBounds(210, 100, 120, 30);
        loadButton.setBounds(210, 140, 120, 30);
    ...
    private class Board extends JPanel implements ActionListener, MouseListener {
        ...
        void doSaveGame(){
            ...
            fileDialog.setSelectedFile(gameData);
            ...
            try {
                PrintWriter pw = new PrintWriter(new FileOutputStream(gameData));
                pw.println("Checkers 1.0");
                pw.println(currentPlayer == CheckersData.RED? "Red": "Black");
        ...
        void doLoadGame(){
            ...
            fileDialog.setDialogTitle("Select a file to open");
            ...
            File gameData = fileDialog.getSelectedFile();

            try{
                TextReader in = new TextReader(new FileInputStream(gameData));
                String line = in.getln();
                if (!line.equals("Checkers 1.0")){
                    JOptionPane.showMessageDialog(this, "Incompatible file");
                    return;
                }
                line = in.getln();
                newCurrentPlayer = (line.equalsIgnoreCase("black"))? 3:1;
                char c;
                for(int row = 0; row < 8; row++){
                    for(int col = 0; col < 8; col++){
                        ...
                        switch (c){
                            case '.':
                                newBoard.board[row][col] = CheckersData.EMPTY;
                                break;
                            case 'r':
                                newBoard.board[row][col] = CheckersData.RED;
                                break;
                            case 'R':
                                newBoard.board[row][col] = CheckersData.RED_KING;
                                break;
                            case 'b':
                                newBoard.board[row][col] = CheckersData.BLACK;
                                break;
                            case 'B':
                                newBoard.board[row][col] = CheckersData.BLACK_KING;
                                break;
                            default:
                                JOptionPane.showMessageDialog(this, "Corrupt board file");
                                return;
                        }
