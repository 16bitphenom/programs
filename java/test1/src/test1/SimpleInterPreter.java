...
public class SimpleInterPreter {
    ...
    private static enum Functions {SIN, COS, TAN, ABS, SQRT, LOG};
    ...
    private static class StandardFunction{
        ...
        public double evaluate(double x){
            Functions fc = this.functionCode;
            switch(fc){
                case SIN:
                    return Math.sin(x);
                case COS:
    ...
    private static double primaryValue() throws ParseError {
        ...
        if ( Character.isDigit(ch) ) {
            ...
            else if (val instanceof StandardFunction){
                ch = TextIO.peek();
                if (ch == '('){
                    TextIO.getAnyChar();
                    double x = expressionValue();
                    TextIO.skipBlanks();
                    if (TextIO.peek() != ')'){
                        throw new ParseError("Missing right parenthesis");
                    }
                    TextIO.getAnyChar();
                    return ((StandardFunction) val).evaluate(x);
                }
                else{
                    throw new ParseError("Illegal statement");
                }
            }
