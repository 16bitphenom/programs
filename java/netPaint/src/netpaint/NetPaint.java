...
public class NetPaint extends JFrame {
    ...
    private class SimplePaintPanel extends JPanel {
        ...
        private JButton listenButton, connectButton, closeButton, sendButton;
        ...
        public SimplePaintPanel() {
            ...
            sendBar.setLayout(new FlowLayout(FlowLayout.CENTER, 3, 3));
            ...
            sendButton = new JButton("Send Drawing");
        ...
        private class ConnectionHandler extends Thread{
            ...
            private ObjectOutputStream outB = null;
            private ObjectInputStream inB = null;
            ...
            synchronized void sendPaintings(){
                ...
                try{
                    ArrayList<Object> cargo = new ArrayList<Object>();
                    if (outB == null){
                        outB = new ObjectOutputStream(socket.getOutputStream());
                    }
                    
                    outB.reset();
                    String handShake = "NETPAINT";
                    cargo.add(handShake);
                    
                    cargo.add(getBackground());
                    cargo.add(new Integer(curves.size()));
                    ArrayList<CurveData> cd = curves;
                    cargo.add(curves);
    
                    System.out.println("Sent " + cd.size());
                    outB.writeObject(cargo);
                    outB.flush();
                    //outB.close();
                    JOptionPane.showMessageDialog(NetPaint.this, "Success");
                }
                catch(IOException e){
                    JOptionPane.showMessageDialog(NetPaint.this, 
                            "Error in writing to the connected computer");
                    return;
                }
                
            }
            
            synchronized void receiveObject(Object receivedCargo){
                if (state != ConnectionState.CONNECTED){
                    JOptionPane.showMessageDialog(NetPaint.this, 
                            "Not connected");
                    return;
                ...
                try{
                    ArrayList<Object> cargo = (ArrayList<Object>)receivedCargo;
                    String handshake = (String)cargo.get(0);
                    newBGColor = (Color)cargo.get(1);
                    int newCount = (Integer)cargo.get(2);
                    System.out.println("Hi " + newCount);
                    ArrayList<CurveData> newCurves = new ArrayList<CurveData>();
                    
                    newCurves = (ArrayList<CurveData>)cargo.get(3);
                    System.out.println(newCurves.size());
                    curves = newCurves;
            ...
            synchronized private void connectionOpened(String message) throws IOException{
                listener = null;
                ...
                outB = new ObjectOutputStream(socket.getOutputStream());
                inB = new ObjectInputStream(socket.getInputStream());
            }
            ...
            public void run(){
                ...
                try{
                    ...
                    if(state == ConnectionState.LISTENING){
                        ...
                        listener.close();
                        message = "Connected to " + socket.getInetAddress();
                        //inB = new ObjectInputStream(socket.getInputStream());
                        //outB = new ObjectOutputStream(socket.getOutputStream());
                    }
                    else if(state == ConnectionState.CONNECTING){
                        socket = new Socket(this.host, this.port);
                        message = "Connected to " + socket.getInetAddress() + ":" + port;
                        
                        
                    }
                    connectionOpened(message);
                    
                    while(state == ConnectionState.CONNECTED){
                       
                        Object testCargo = inB.readObject();
                        if (testCargo == null){
                             connectionClosedFromOtherSide();
                        }
                        else{
                            System.out.println("Received");
                            receiveObject(testCargo);
                        }
                    }
                }
                catch(Exception e){
